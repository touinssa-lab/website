...
// --- DATA PARSING FROM RECOVERED TS FILE ---

function parseKeywords() {
  const content = fs.readFileSync(path.join(process.cwd(), 'src/data/aiHotKeywords.ts'), 'utf8');
  const keywordsMatch = content.match(/export const webTrendKeywords: WebTrendKeyword\[\] = \[([\s\S]*?)\];/);
  if (!keywordsMatch) return [];
  
  const arrayContent = keywordsMatch[1];
  // Simple regex based parsing for this specific format
  const items = [];
  const regex = /\{ id: '(\d+)', section: '(.*?)', rank: (\d+), keyword: '(.*?)', interest: (\d+), change: '(.*?)' \}/g;
  let m;
...
async function restore() {
  console.log('Starting full restore for 2026-05-11...');

  // 1. Restore Insights (Fixed column name to 'reason')
  ...
  if (keywords.length > 0) {
    ...
    for (let i = 0; i < keywords.length; i += 100) {
      ...
      const { error: kwError } = await supabase
        ...
        .upsert(chunk);
      if (kwError) {
        console.error(`Keyword Restore Error at chunk ${i/100}:`, kwError);
      } else {
        console.log(`Keywords chunk ${i/100} restored.`);
      }
    }
  }
