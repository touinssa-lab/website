...
def process_excel(file_path):
    ...
    for i, sheet_name in enumerate(sheets):
        ...
        try:
            df = pd.read_excel(file_path, sheet_name=sheet_name, header=1) # usually header is at row 1
...
file_path = 'scratch/trends/2026-05-15.xlsx'
if os.path.exists(file_path):
    keywords = process_excel(file_path)
    with open('scratch/trends/2026-05-15_keywords.json', 'w', encoding='utf-8') as f:
        json.dump(keywords, f, ensure_ascii=False, indent=2)
    print(f"Processed {len(keywords)} keywords. Saved to JSON.")
else:
